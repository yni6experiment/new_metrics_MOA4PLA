...
public class Experiment {
	...
	while (r.next()) {
	    ...
	    //addYni
	    allMetrics.setWocsclass(buildWocsclass(connection, exec, exp));
	    allMetrics.setWocsinterface(buildWocsinterface(connection, exec, exp));
	    allMetrics.setSsc(buildSsc(connection, exec, exp));
	    allMetrics.setSvc(buildSvc(connection, exec, exp));
	    allMetrics.setAv(buildAv(connection, exec, exp));
    ...
    private static List<Wocsclass> buildWocsclass(Connection connection, Execution exec, Experiment exp)
    	    throws SQLException {
    	Statement wocscStatement = connection.createStatement();
    	StringBuilder query = new StringBuilder();
    	query.append("select * from WocsclassMetrics where execution_id=");
    	query.append(exec.getId());
    	query.append(" OR (execution_id='' AND experiement_id=");
    	query.append(exp.getId());
    	query.append(")");

    	ResultSet resultWocsc = wocscStatement.executeQuery(query.toString());

    	List<Wocsclass> wclass = new ArrayList<Wocsclass>();

    	while (resultWocsc.next()) {
    	    String idSolution = resultWocsc.getString("id_solution");
    	    Wocsclass wc = new Wocsclass(idSolution, exec, exp);
    	    wc.setWocsClass(getResultParseFloat(resultWocsc, "wocsClass"));
    	    wc.setIsAll(getResultParseInteger(resultWocsc, "is_all"));
    ...
    private static List<Wocsinterface> buildWocsinterface(Connection connection, Execution exec, Experiment exp)
    	    throws SQLException {
    	Statement wocsiStatement = connection.createStatement();
    	StringBuilder query = new StringBuilder();
    	query.append("select * from WocsinterfaceMetrics where execution_id=");
    	query.append(exec.getId());
    	query.append(" OR (execution_id='' AND experiement_id=");
    	query.append(exp.getId());
    	query.append(")");

    	ResultSet resultWocsi = wocsiStatement.executeQuery(query.toString());

    	List<Wocsinterface> winterface = new ArrayList<Wocsinterface>();

    	while (resultWocsi.next()) {
    	    String idSolution = resultWocsi.getString("id_solution");
    	    Wocsinterface wi = new Wocsinterface(idSolution, exec, exp);
    	    wi.setWocsInterface(getResultParseFloat(resultWocsi, "wocsInterface"));
    	    wi.setIsAll(getResultParseInteger(resultWocsi, "is_all"));
    ...
    private static List<Ssc> buildSsc(Connection connection, Execution exec, Experiment exp)
    	    throws SQLException {
    	Statement sscStatement = connection.createStatement();
    	StringBuilder query = new StringBuilder();
    	query.append("select * from SscMetrics where execution_id=");
    	query.append(exec.getId());
    	query.append(" OR (execution_id='' AND experiement_id=");
    	query.append(exp.getId());
    	query.append(")");

    	ResultSet resultSsc = sscStatement.executeQuery(query.toString());

    	List<Ssc> ssc = new ArrayList<Ssc>();

    	while (resultSsc.next()) {
    	    String idSolution = resultSsc.getString("id_solution");
    	    Ssc ss = new Ssc(idSolution, exec, exp);
    	    ss.setSsc(getResultParseFloat(resultSsc, "ssc"));
    	    ss.setIsAll(getResultParseInteger(resultSsc, "is_all"));
    ...
    private static List<Svc> buildSvc(Connection connection, Execution exec, Experiment exp)
    	    throws SQLException {
    	Statement svcStatement = connection.createStatement();
    	StringBuilder query = new StringBuilder();
    	query.append("select * from SvcMetrics where execution_id=");
    	query.append(exec.getId());
    	query.append(" OR (execution_id='' AND experiement_id=");
    	query.append(exp.getId());
    	query.append(")");

    	ResultSet resultSvc = svcStatement.executeQuery(query.toString());

    	List<Svc> svc = new ArrayList<Svc>();

    	while (resultSvc.next()) {
    	    String idSolution = resultSvc.getString("id_solution");
    	    Svc sv = new Svc(idSolution, exec, exp);
    	    sv.setSvc(getResultParseFloat(resultSvc, "svc"));
    	    sv.setIsAll(getResultParseInteger(resultSvc, "is_all"));
    ...
    private static List<Av> buildAv(Connection connection, Execution exec, Experiment exp)
    	    throws SQLException {
    	Statement avStatement = connection.createStatement();
    	StringBuilder query = new StringBuilder();
    	query.append("select * from AvMetrics where execution_id=");
    	query.append(exec.getId());
    	query.append(" OR (execution_id='' AND experiement_id=");
    	query.append(exp.getId());
    	query.append(")");

    	ResultSet resultAv = avStatement.executeQuery(query.toString());

    	List<Av> av = new ArrayList<Av>();

    	while (resultAv.next()) {
    	    String idSolution = resultAv.getString("id_solution");
    	    Av v = new Av(idSolution, exec, exp);
    	    v.setAv(getResultParseFloat(resultAv, "av"));
    	    v.setIsAll(getResultParseInteger(resultAv, "is_all"));
    ...
    //addYni
    private static Float getResultParseFloat(ResultSet resultSetFloat, String nameColumn) {
    	try {
    	    return Float.parseFloat(resultSetFloat.getString(nameColumn));
    	} catch (NumberFormatException | SQLException e) {
    	    e.printStackTrace();
    	}
    	return null;
